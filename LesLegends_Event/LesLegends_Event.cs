...
namespace LesLegends_Event
{
    ...
    public class LesLegends_Event : TaiwuRemakePlugin
    {
        ...
        public override void Initialize()
        {
            ...
            eventHarmony ??= new Harmony(base.GetGuid());
        }
        ...
        {
            eventHarmony.UnpatchAll();
            eventHarmony.PatchAll(typeof(EventPatch));
            return true;
        }

        /// <summary>
        /// 启动时进行事件的patch会有问题，应该是还未加载进来
        /// 改成在世界加载时再进行，所以单独拿一个类封装一下patch的方法，等加载的时候对这个class进行PatchAll即可
        /// </summary>
        public static class EventPatch {
            ...
            public static void TaiwuEventMarray_OnOption10VisibleCheck_Postfix(TaiwuEvent_bad63f08115a45aa970cfa203dd85e2b __instance, ref bool __result)
            {
                ...
                if ((modifyForDifSexual || taiwu.GetGender() == target.GetGender()) && (unlimitMarray || unlimitMonk || unlimitBlood))   // (修改对异性恋生效 || 为同性) 设置需要修改
                {
                        ...
                        && EventHelper.CheckHasRelationship(taiwu, target, RelationType.Adored) && EventHelper.CheckHasRelationship(target, taiwu, RelationType.Adored))    // 互相倾慕
                    {
                        ...
                        __result = (favorabilityTypeToTarget >= minSelfFavorabilityReq);
                        /*__result = true;*/
                    }
                }
            }
            ...
            // 比武招亲入口事件统一处理
            public static void CommonMarrayEventEntryHandler(ref string __result, Character taiwu, Character target, string entrance, string tooYougn, string femaleNotFit, string notFemaleNotFit)
            {
            ...
            // 比武招亲结束事件统一处理
            public static void CommonMarrayEventFinishHandler(ref string __result, Character taiwu, Character target, string success, string genderFailure, string married)
            {
